-- Считает расчётную выручку по услугам за период из оплаченных платежей.
-- Сумма платежа делится поровну между услугами, подключёнными к SIM на момент оплаты.
-- Таблицы: payments, sim_card_services, services.
