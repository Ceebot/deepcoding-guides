...
- **Суть:** при постановке группы в комнату из импорта Excel в `UPDATE pork_group` не сбрасывался `is_slaughter`; возможна комбинация «группа в зале» + флаг убоя. В SQL добавлен `is_slaughter = false` вместе с `pork_room_id` (как в `PlacedGroupController` и отгрузке в зал в `ShippedGroupController`).
...
- **Суть:** при отгрузке группы из таблицы без комнаты можно выбрать «На убой»; в БД `pork_group.is_slaughter`; на главной в списке групп без комнаты отображается суффикс `(убой)`; при постановке в зал и при отгрузке в другую комнату флаг сбрасывается. Колонка `is_slaughter` добавлена и в `poultry_group` (симметрия схемы, UI только PORK).
...
- **Суть:** над секциями карточек на `/` (список комнат/корпусов) выводится строка с перечислением активных групп без привязки к комнате (PORK: `pork_group.pork_room_id IS NULL` и `end_date IS NULL`) или без корпуса (POULTRY: `poultry_group.building_id IS NULL` и `end_date IS NULL`); при отсутствии групп — плейсхолдер «нет».
...
- **Суть:** для операций постановки/отгрузки у PORK реализован приоритет ручного возраста (`Кол-во дней`) с fallback на автопересчёт от даты группы; при движениях обновляется `pork_group.start_date`; в header комнаты возраст считается по формуле: `возраст на момент постановки + прошедшие дни с даты постановки`.
...
- **Суть:** на главной свиноводства заголовок и подзаголовок приведены к формату страницы птицеводства (с формулировкой про свиней), а в сайдбаре на этой странице теперь отображается `Список корпусов` вместо `Участок доращивания`.
...
- **Суть:** на главной свиноводства отображался заголовок уровня площадки (`Репродуктор ...`), потому что секции были сгруппированы по `pork_site`. Для соответствия таблице свиноводства группировка переведена на уровень корпуса/участка (`pork_building`: `Доращивание 1/2`, `Откорм 1/2`).
...
- **Суть:** на странице `/` в режиме POULTRY список корпусов сгруппирован по участкам: сначала заголовок участка, затем карточки корпусов этого участка; для пустого названия используется fallback `Без участка`.
...
- **Суть:** на странице `/` карточки комнат сгруппированы по участкам: сначала заголовок участка, ниже — относящиеся к нему комнаты; добавлен fallback-заголовок `Без участка` для комнат без привязки к участку.
...
- **Суть:** в `PorkGroup\TableData::applyLpmData` при пустом `medicament_id` в интервале — `continue`, как у птицы; `SavedPoultryEggsDay::clearDayForDate` сбрасывает только `whole`, `defective` сохраняется; контракт `api-contracts.md` (eggs); парсер ЛПМ — один пункт про `history`, термин «метафилактика»; PHPDoc `MedicamentsController` про `animal_type IS NULL` при фильтре; юнит-тесты `TableDataApplyLpmTest`, доработка `SavedPoultryEggsDayTest`.
...
- **Суть:** в `applyLpmData` пустой `medicament_id` не прерывает перебор ЛПМ; миграция `Version20260421153100` больше не затирает `NULL` в `medicament.animal_type` (общие препараты); `PUT` eggs с `records: []` — через `clearDayForDate` снимается только whole, defective не трогается (ранее в этой строке неверно «очищает день»; актуально — раунд 2 и `api-contracts.md`); в модалке яиц в запрос не уходят полностью пустые блоки; один `#[Route]` у `SavedCullingCellController`; PHPDoc по агрегации корма; уточнены `api-contracts.md`, описание парсера ЛПМ, тесты `PoultryTableDataTest`, `SavedPoultryEggsDayTest`.
